...
class GameViewController: UIViewController, GameProtocol {
    ...
    var livesRemaining = 10 {
        didSet {
            guessesRemainingLabel.text = "\(livesRemaining) restantes"
        }
    }
    ...
    func checkToSeeIfCompleted() {
        ...
        if livesRemaining > 0 {
            if maskedWord == word {
                showAlertAction(title: "Parabéns 🎉", message: "Você venceu o carrasco!", actionTitle: "Jogar de novo", actionClosure: self.loadWord)
                playSound(sound: K.Audio.gameWonSound)
                nextLevel()
            }
            
        } else {
            showAlertAction(title: "💀", message: "O carrasco te pegou, a palavra era \"\(word.uppercased())\"!", actionTitle: "Jogar de novo", actionClosure: self.loadWord)
            nextLevel()
        }
